export class Attachment {
  ...
  static from(files: File[] | Attachment[] | FileList): Attachment[] {
    ...
    for (const file of files as Attachment[]) {
...
function transferredFiles(
  ...
  return Promise.resolve(
    visible(Array.from(transfer.files || [])).map((f) => new Attachment(f))
  );
}
...
async function traverse(
  ...
  for (const entry of visible(entries)) {
    if (entry.isDirectory) {
      results.push(
        ...(await traverse(entry.fullPath, await getEntries(entry as any)))
      );
    } else {
...
function isDirectory(transfer: DataTransfer): boolean {
  return (
    ...
    Array.from(transfer.items).some((item) => {
      const entry = 'webkitGetAsEntry' in item && item.webkitGetAsEntry();
